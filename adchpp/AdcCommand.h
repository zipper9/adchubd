...
namespace adchpp
{
	...
	public:
		...
		// Known commands...
#define C(n, a, b, c)                                                                               \
	static const uint32_t CMD_##n = (((uint32_t)a) | (((uint32_t)b) << 8) | (((uint32_t)c) << 16)); \
	typedef Type<CMD_##n> n
		// Base commands
		C(SUP, 'S', 'U', 'P');
		C(STA, 'S', 'T', 'A');
		C(INF, 'I', 'N', 'F');
...
#undef C
		...
		{
			return std::string(reinterpret_cast<const char*>(&aSID), sizeof(aSID));
		}
		static void appendSID(std::string& str, uint32_t aSID)
		{
			str.append(reinterpret_cast<const char*>(&aSID), sizeof(aSID));
		}
		...
		{
			return toCMD(str[0], str[1], str[2]);
		}
		...
		{
			return std::string(reinterpret_cast<const char*>(&aField), sizeof(aField));
		}
		...
		explicit AdcCommand(const std::string& aLine)
		: cmdInt(0), priority(PRIORITY_NORMAL), from(INVALID_SID), to(INVALID_SID), type(0)
		{
			parse(aLine);
		}
		explicit AdcCommand(const BufferPtr& buffer_)
		: buffer(buffer_), cmdInt(0), priority(PRIORITY_NORMAL), from(INVALID_SID), to(INVALID_SID),
		  type(0)
		{
	...
	public:
		...
		{
#define C(n)                                                \
	case AdcCommand::CMD_##n:                               \
		return ((T*)this)->handle(AdcCommand::n(), c, cmd); \
		break;
