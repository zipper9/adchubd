...
namespace adchpp
{
	...
	void Util::decodeUrl(const string& url, string& aServer, short& aPort, string& aFile)
	{
		...
		{
			// Port
			if (k < j) aPort = (short)Util::toInt(url.substr(k + 1, j - k - 1));
		}
		else
		{
			k = j;
	...
	void Util::tokenize(StringList& lst, const string& str, char sep, string::size_type j)
	{
		...
		if (j <= str.size()) lst.push_back(str.substr(j, str.size() - j));
	}
	...
	string Util::getAppPath()
	{
		...
		if (i != string::npos) tmp.erase(i + 1);
		return tmp;
	}
	...
	string Util::translateError(int aError)
	{
		...
		::FormatMessage(FORMAT_MESSAGE_ALLOCATE_BUFFER | FORMAT_MESSAGE_FROM_SYSTEM | FORMAT_MESSAGE_IGNORE_INSERTS,
						NULL, aError, MAKELANGID(LANG_NEUTRAL, SUBLANG_DEFAULT), // Default language
...
#ifdef __MINGW32__
	// creating a random_device throws an exception in MinGW with GCC 4.6; simpler
	// alternative...
	uint32_t Util::rand()
	{
