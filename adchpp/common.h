 ...
 * @mainpage
 *
 * ADCH++ is a server application meant to be used in the Direct Connect
 * network, released under the GPL-2 license.
 *
...
#define dcassert(exp)                                    \
	do                                                   \
	{                                                    \
		if (!(exp)) logAssert(__FILE__, __LINE__, #exp); \
	} while (false)
...
#define dcdebug \
	if (false) printf
//#define dcassert(exp) do { if(!(exp)) logAssert(__FILE__, __LINE__, #exp); }
// while(0)
#define dcassert(exp)
#ifdef _MSC_VER
