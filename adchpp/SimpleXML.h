...
namespace adchpp
{
	...
	public:
		...
		template <typename T> void addAttrib(const std::string& aName, const T& aData)
		{
			addAttrib(aName, Util::toString(aData));
		}

		void addAttrib(const std::string& aName, const std::string& aData);

		template <typename T> void addChildAttrib(const std::string& aName, const T& aData)
		{
			addChildAttrib(aName, Util::toString(aData));
		}
		void addChildAttrib(const std::string& aName, const std::string& aData);
		...
		bool findChild(const std::string& aName) const noexcept;
		...
		const std::string& getChildAttrib(const std::string& aName,
										  const std::string& aDefault = Util::emptyString) const
		{
			checkChildSelected();
		...
		/**
		 * This is a heurestic for whether escape needs to be called or not. The
		 * results are only guaranteed for false, i e sometimes true might be returned
		 * even though escape was not needed...
		 */
		static bool needsEscape(const std::string& aString, bool aAttrib, bool aLoading = false)
		{
			return ((aLoading) ? aString.find('&') : aString.find_first_of(aAttrib ? "<&>'\"" : "<&>")) !=
				   std::string::npos;
		}
	...
	private:
		...
		public:
			...
			/**
			 * A simple list of children. To find a tag, one must search the entire
			 * list.
			 */
			List children;
			/**
			 * Attributes of this tag. According to the XML standard the names
			 * must be unique (case-sensitive). (Assuming that we have few attributes
			 * here, we use a vector instead of a (hash)map to save a few bytes of
			 * memory and unnecessary calls to the memory allocator...)
			 */
			AttribMap attribs;
			...
			: name(aName), data(aData), parent(aParent)
			{
