 ...
 * for our filter is m = n * k / ln(2) where n is the number of TTH values, or in our case, number
 * of files in share since each file is identified by one TTH value. We try that for each even
 * dividend of the key size (2, 3, 4, 6, 8, 12) and if m fits within the bits we're able to address
 * (2^(keysize/k)), we can use that value when requesting the bloom filter.
 */
class HashBloom
{
public:
