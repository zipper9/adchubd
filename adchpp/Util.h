...
namespace adchpp
{
	 ...
	 */
	template <typename T1> inline int compare(const T1& v1, const T1& v2)
	{
	...
	public:
		...
		{
			char buf[64];
			sprintf(buf, "%01d:%02d:%02d:%02d", (int)(aSec / (24 * 60 * 60)),
					(int)((aSec / (60 * 60)) % 24), (int)((aSec / 60) % 60), (int)(aSec % 60));
			return buf;
		}

		static bool toBool(const std::string& aString)
		{
			return toBool(aString.c_str());
		}
		...
		{
			return toDouble(aString.c_str());
		}
		static float toFloat(const std::string& aString)
		{
			return toFloat(aString.c_str());
		}
