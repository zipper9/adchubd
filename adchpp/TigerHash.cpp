...
#define round(a, b, c, x, mul)                                                                   \
	...
	a -= t1[(uint8_t)(c)] ^ t2[(uint8_t)(((uint32_t)(c)) >> (2 * 8))] ^                          \
		 t3[(uint8_t)((c) >> (4 * 8))] ^ t4[(uint8_t)(((uint32_t)((c) >> (4 * 8))) >> (2 * 8))]; \
	b += t4[(uint8_t)(((uint32_t)(c)) >> (1 * 8))] ^ t3[(uint8_t)(((uint32_t)(c)) >> (3 * 8))] ^ \
		 t2[(uint8_t)(((uint32_t)((c) >> (4 * 8))) >> (1 * 8))] ^                                \
		 t1[(uint8_t)(((uint32_t)((c) >> (4 * 8))) >> (3 * 8))];                                 \
	b *= mul;

#define pass(a, b, c, mul)                                                                          \
	round(a, b, c, x0, mul) round(b, c, a, x1, mul) round(c, a, b, x2, mul) round(a, b, c, x3, mul) \
	round(b, c, a, x4, mul) round(c, a, b, x5, mul) round(a, b, c, x6, mul) round(b, c, a, x7, mul)
...
#define compress                                                  \
	save_abc for (pass_no = 0; pass_no < PASSES; pass_no++)       \
	{                                                             \
		if (pass_no != 0)                                         \
		{                                                         \
			key_schedule                                          \
