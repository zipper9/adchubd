...
namespace adchpp
{
	...
	File::File(const string& aFileName, int access, int mode)
	{
		...
		int a = 0;
		if (access & READ) a |= GENERIC_READ;
		if (access & WRITE) a |= GENERIC_WRITE;
	...
	int64_t File::getSize()
	{
		...
		if ((l == INVALID_FILE_SIZE) && (GetLastError() != NO_ERROR)) return -1;
	...
	File::File(const string& aFileName, int access, int mode)
	{
		...
		if (h == -1) throw FileException("Could not open file");
	}

	int64_t File::getSize()
	{
		struct stat s;
		if (fstat(h, &s) == -1) return -1;
	...
	{
		struct stat s;
		if (stat(aFileName.c_str(), &s) == -1) return -1;
