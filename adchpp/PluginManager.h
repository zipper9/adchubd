 ...
 *
 * ADCH++ contains a rather powerful plugin API that can be used to create
 * advanced plugins that change or add to ADCH++'s behaviour. Most plugins will
 * need PluginManager.h, ClientManager.h and Client.h included to work, even
 * though the other header files are available as well (they're more likely to
 * change in future versions though). You can use any method that is declared as
 * DLL or is inline, the others are meant to be internal to ADCH++, very likely
 * to change/disappear and will generate link errors (when compiling under
 * windows anyway). When starting a plugin project I strongly recommend that you
 * take one of the existing plugins and modify it to your needs (to get all
 * compiler settings and base code right).
 *
 * @section Versions Versions
 *
 * Due to C++ name mangling, plugins are generally valid only for a certain
 * version of the ADCH++ plugin API. This version usually follows the main
 * ADCH++ version, unless a small update is made that I judge shouldn't affect
 * plugins in any way. Most of the time, recompiling the plugin should be
 * enough, unless any major changes have been made, and your plugin doesn't rely
 * on the nasty internals.
 *
 ...
 * multithreading issues unless you start threads by yourself. Second, any work
 * you do in a plugin halts <b>all</b> of ADCH++'s processing (apart from
 * receiving/sending buffered data), in other words, don't do any lengthy
 * processing in the on methods, as the whole of ADCH++ will suffer. Third, if
 * you indeed start another thread, make sure you don't use any API functions
 * from it apart from those explicitly marked as thread safe. To indicate from a
 * plugin that you have work to do in the main worker thread, call
...
#define PLUGIN_API __attribute__((visibility("default")))
	 ...
	 * @return 0 if the plugin was loaded ok, != 0 otherwise (the number will be
	 * logged, use as error code). Plugin dll will get unloaded without calling
	 * pluginUnload if the return value is not 0 here.
	 * @see pluginUnload
	 */
	...
	public:
		...
		{
			return PluginDataHandle(new PluginData(deleter_));
		}
		 ...
		 * The parameters are the same as ClientManager::signalReceive, only that the
